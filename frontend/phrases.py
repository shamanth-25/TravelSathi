...
if phrases_list:
    ...
    for p in phrases_list:
        if isinstance(p, str):
            # Fallback if LLM generates a flat list of strings instead of dicts
            if " - " in p:
                parts = p.split(" - ", 1)
                if len(parts) == 2:
                    trans_part = parts[0].strip()
                    phrase_part = parts[1].strip()
                    pron = "N/A"
                    ...
                    if "(" in trans_part and ")" in trans_part:
                        ...
                        trans_part = trans_clean
                        pron = pron_clean
                        
                    df_data.append({
                        t('phrases_table_phrase'): phrase_part,
                        t('phrases_table_translation'): trans_part,
                        t('phrases_table_pronunciation'): pron
                    })
                else:
                    df_data.append({
                        t('phrases_table_phrase'): p,
                        t('phrases_table_translation'): p,
                        t('phrases_table_pronunciation'): "-"
                    })
            else:
                df_data.append({
                    t('phrases_table_phrase'): p,
            ...
            if not phrase_val and not trans_val and not pron_val:
                vals = list(p.values())
                phrase_val = vals[0] if len(vals) > 0 else "-"
                trans_val = vals[1] if len(vals) > 1 else phrase_val
                pron_val = vals[2] if len(vals) > 2 else "-"
            else:
                phrase_val = phrase_val or "-"
                trans_val = trans_val or phrase_val
                pron_val = pron_val or "-"
                
            df_data.append({
                t('phrases_table_phrase'): phrase_val,
                t('phrases_table_translation'): trans_val,
                t('phrases_table_pronunciation'): pron_val
            })
    ...
    # Render in rows of 3 columns to prevent squeezing
    cols_per_row = 3
